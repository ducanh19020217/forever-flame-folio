...
const Hero = ({
    ...
    // 👇 Scroll 2 nhịp để tránh lệch vị trí khi layout chưa ổn định
    const smoothScrollTo = (targetId: string) => {
        const el = document.getElementById(targetId);
        if (!el) return;

        const getTargetTop = () => {
            const nav = document.querySelector("nav"); // nếu có navbar fixed
            const navH = nav instanceof HTMLElement ? nav.offsetHeight : 64;
            return el.offsetTop - navH - 8;
        };

        // Nhịp 1: scroll ngay lập tức (smooth)
        window.scrollTo({
            top: getTargetTop(),
            behavior: "smooth",
        });

        // Nhịp 2: sau 500ms khi layout đã ổn định, chốt lại vị trí
        setTimeout(() => {
            window.scrollTo({
                top: getTargetTop(),
                behavior: "instant" as ScrollBehavior, // không smooth để không thấy nhảy
            });
        }, 500);
    };
    ...
    return (
        ...
        >
            ...
            <div
                className={`relative z-10 text-center px-4 py-20 pt-[10rem] max-w-4xl mx-auto transition-all duration-1000 delay-300 ${
            ...
            >
                ...
                <h1 className="text-5xl md:text-7xl lg:text-8xl font-bold mb-6 text-white text-center leading-tight drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]">
                    <div className="flex flex-col lg:flex-row justify-center items-center" style={{
                        fontFamily: '"Meow Script", cursive',
                        fontSize: '76px'
                    }}>
                        <span className="whitespace-nowrap">{brideName}</span>
                        <span className="text-primary mx-4 drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]" style={{lineHeight: '40px'}}>
              &
            </span>
